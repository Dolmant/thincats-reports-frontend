      ...
      try {
        ...
        if (!pseudo_total) {
          ...
          if (content_length) {
            ...
            var content_encoding = original_event.target.getResponseHeader(
              "content-encoding"
            );
            if (content_encoding && content_encoding != "identity") {
              pseudo_total =
                (content_length * defaults.CONTENT_ENCODING_MULTIPLE) | 0;
            } else {
            ...
            event = new Proxy(event, {
              get: function(target, name) {
                if (name == "lengthComputable") {
                  ...
                  if (!pseudo_total) {
                    pseudo_total = calculate_pseudo_total(
                      defaults,
                      original_event
                    );
                  }
    ...
    var proxy = {
      set: function(target, name, value) {
        if (
          ...
          name.indexOf("on") == 0 &&
          target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)]
        ) {
          if (
            target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)][
              "on"
            ...
            delete target.xmlHTTPRequestLengthComputable["listeners"][
              name.substr(2)
            ]["on"];
          }
          if (value) {
            target[name] = onprogress(
              target.xmlHTTPRequestLengthComputable,
              value
            );
            target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)][
        ...
        if (name == "addEventListener") {
          return function() {
            if (
              ...
              target.xmlHTTPRequestLengthComputable.listeners[arguments[0]][
                "listeners"
              ].push([event_listener, arguments[1]]);
