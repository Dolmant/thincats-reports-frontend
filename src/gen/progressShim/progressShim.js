...
(function () {
  ...
  if (window.Proxy) {
    ...
    var calculate_pseudo_total = function (defaults, original_event) {
      ...
      try {
        if (!pseudo_total) {
          var real_length = original_event.target.getResponseHeader(defaults.DECOMPRESSED_CONTENT_LENGTH_HEADER);
          if (real_length != undefined) {
            pseudo_total = parseInt(real_length);
          }
        }
        if (!pseudo_total) {
          var content_length = original_event.target.getResponseHeader("content-length");
          if (content_length) {
            content_length = parseInt(content_length);
            var content_encoding = original_event.target.getResponseHeader("content-encoding");
            if (content_encoding && content_encoding != "identity") {
              pseudo_total = content_length * defaults.CONTENT_ENCODING_MULTIPLE | 0;
            } else {
    ...
    var onprogress = function (defaults, callback) {
      ...
      return function (event) {
        if (event && (event instanceof ProgressEvent || defaults.no_type_check) && (event.type == "progress" || event.type == "load" || event.type == "loadend")) {
            ...
            event = new Proxy(event, {
              get: function get(target, name) {
                if (name == "lengthComputable") {
                  ...
                  if (!pseudo_total) {
                    pseudo_total = calculate_pseudo_total(defaults, original_event);
                  }
        ...
        if (callback) {
          callback.bind(defaults.xmlhttprequest)(event);
        }
      };
    };
    ...
    var proxy = {
      set: function set(target, name, value) {
        if (target.xmlHTTPRequestLengthComputable && name.indexOf("on") == 0 && target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)]) {
          if (target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)]["on"]) {
            delete target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)]["on"];
          }
          if (value) {
            target[name] = onprogress(target.xmlHTTPRequestLengthComputable, value);
            target.xmlHTTPRequestLengthComputable["listeners"][name.substr(2)]["on"] = value;
          } else {
            target[name] = value;
          }
        } else {
        ...
        if (name == "addEventListener") {
          return function () {
            if (target.xmlHTTPRequestLengthComputable["listeners"][arguments[0]]) {
              var event_listener = onprogress(target.xmlHTTPRequestLengthComputable, arguments[1]);
              target.xmlHTTPRequestLengthComputable.listeners[arguments[0]]["listeners"].push([event_listener, arguments[1]]);
              return target[name].call(target, arguments[0], event_listener, arguments[2], arguments[3]);
            } else {
          ...
          return function () {
            if (target.xmlHTTPRequestLengthComputable.listeners[arguments[0]]) {
              var listeners = target.xmlHTTPRequestLengthComputable.listeners[arguments[0]]["listeners"];
              for (var i = 0; i < listeners.length; ++i) {
                if (listeners[i][1] == arguments[1]) {
                  var listener = listeners.splice(i)[0];
                  return target[name].call(target, arguments[0], listener[0], arguments[2], arguments[3]);
                }
              }
            } else {
              return target[name].apply(target, arguments);
    ...
    window.XMLHttpRequest = function (arg) {
      ...
      if (config["decompressed-content-length"]) {
        defaults["decompressed-content-length"] = config["decompressed-content-length"];
      }
      defaults["listeners"] = {
        progress: { listeners: [] },
        load: { listeners: [] },
      ...
      xmlhttprequest.addEventListener("load", function () {
        ...
        if (defaults["latestProgress"]) {
          if (defaults["listeners"]["progress"]["on"]) {
            defaults["listeners"]["progress"]["on"](defaults["latestProgress"]);
          }
          var progress_listeners = defaults["listeners"]["progress"]["listeners"];
